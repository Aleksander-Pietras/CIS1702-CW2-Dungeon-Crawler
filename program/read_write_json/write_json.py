...
def update_room(room):
    '''Updates a room that already exists
    This cannot be used to update the name of an existing room.'''
    filename = "database.json"
    path = file_path_finder_all(filename)
    current_data = read_json(filename)

    try:
        current_data["rooms"][room["name"]]=room
        with open(path, mode="w") as f:
            json.dump(current_data, f, indent=4)
            print("The room has been updated!")
    except:
        print("Sorry, that room could not be found. If you are trying to add a new room, please use overwrite_json()")
...
if __name__ == "__main__":
    '''FILENAME = "database.json"
    FILE_PATH = file_path_finder_all(FILENAME)''' #Commented out because I don't think we need it.
